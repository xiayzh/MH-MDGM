...
def test(encoder_model,epoch,x32_test,x16_test):
    encoder.eval()
    decoder.eval()

    z = torch.randn(12, 1, 8, 8).to(device)
    _,z16,_ = X_l_encoder(encoder_model,(torch.FloatTensor(x16_test[111:112])).to(device))
    z16 = (torch.ones([12,1,4,4])).to(device)*z16
    z16 = upsample(z16,16)
    ...
    real_imgs = x32_test[[10,30,50,100]]
    real_x16 = x16_test[[10,30,50,100]]
    _,z16,_ = X_l_encoder(encoder_model,(torch.FloatTensor(real_x16)).to(device))
    z16 = upsample(z16,16)
...
def loss_function(recon_x, x, mu1, logvar1):
    BCE = F.mse_loss(recon_x.view(-1,1024), x.view(-1,1024), size_average=False)
...
for epoch in range(1,args.n_epochs+1):
    ...
    if (epoch) % args.sample_interval == 0:
        test(encoder_model,epoch,x32_test,x16_test)
        torch.save(decoder.state_dict(), model_dir + f'/decoder_16_32_VAE_1_{args.beta_vae}_epoch{epoch}.pth')
        torch.save(encoder.state_dict(), model_dir + f'/encoder_16_32_VAE_1_{args.beta_vae}_epoch{epoch}.pth')
